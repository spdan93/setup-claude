...
| TC-S01-02 | TC-1.1-02 | 1. `POST /links` with `{"destinationUrl":"https://example.com"}`. <br> 2. `GET /links/{shortCode}/stats` with valid Bearer token. | Stats response contains `{"clicks": 0, ...}` immediately after creation. | P2 |
...
| TC-S05-01 | TC-1.4-01 | 1. `POST /links` → note `shortCode`. <br> 2. `DELETE /links/{shortCode}` (deactivate). <br> 3. `GET /{shortCode}` without a token. | Step 2 returns `200 OK` with `{"active":false}`. Step 3 returns `410 Gone` with `{"code":"LINK_UNAVAILABLE","message":"This link is unavailable."}`. | P1 |
| TC-S05-02 | TC-1.4-02 | 1. Following TC-S05-01. <br> 2. `PATCH /links/{shortCode}` with `{"active":true}`. <br> 3. `GET /{shortCode}` without a token. | Step 2 returns `200 OK` with `{"active":true}`. Step 3 returns `302 Found` with correct `Location`. Click count from before deactivation is preserved. | P1 |
...
| TC-S09-02 | | 1. `GET /` (root path with no code). | Returns `404 Not Found` or `400 Bad Request`; no unhandled exception or 500. | P3 |
