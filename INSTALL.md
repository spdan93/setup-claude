...
| Peça | Precisa de |
|------|-----------|
| `install.sh` (mac/linux) | **nada a instalar.** Pra mesclar num `settings.json` já existente ele usa, nesta ordem, `jq` → `python3` → `awk` (sempre presente). O `awk` copia o bloco direto — então nunca é preciso instalar `jq`/`python` |
